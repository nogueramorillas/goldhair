...
async function openBookingDetail(id) {
  ...
  document.getElementById('bookingDetailActions').innerHTML = `
    ${b.status === 'confirmed' ? `
      ...
      ${b.client_phone ? `<a class="btn-sm btn-edit" href="${buildWhatsappReminderLink(b)}" target="_blank" rel="noopener noreferrer">💬 Recordatorio WhatsApp</a>` : ''}
...
function buildWhatsappReminderLink(b) {
  const digits = (b.client_phone || '').replace(/[^\d]/g, '');
  const message = `Hola ${b.client_name}, te recordamos tu cita de ${b.service_name} el ${b.date} a las ${b.time_start}. ¡Te esperamos!`;
